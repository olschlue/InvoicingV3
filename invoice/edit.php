...
{
	$Searchstring = "&amp;InvoiceID1=$InvoiceID1&amp;CustomerID1=$CustomerID1&amp;DateFrom1=$DateFrom1&amp;DateTill1=$DateTill1&amp;Total1=$Total1&amp;Customer1=$Customer1&DateAchieved1=$DateAchieved1";
...
if(!isset($tmpID))
{
	...
	$query = $db->Execute("SELECT INVOICEPOSID, MYID, INVOICEID, POSITIONID, POS_DESC, POS_QUANTITY, POS_PRICE, POS_GROUP, TAX, TAX_DESC, TAX_MULTI, TAX_DIVIDE FROM invoicepos WHERE INVOICEID=$invoiceID ORDER BY POS_GROUP ASC, INVOICEPOSID ASC");
...
$query1 = $db->Execute("SELECT MYID, INVOICEID FROM invoice WHERE INVOICEID=$invoiceID");
...
$query2 = $db->Execute("SELECT A.PREFIX, A.TITLE, A.FIRSTNAME, A.LASTNAME, A.ADDRESS, A.COMPANY, A.POSTALCODE, A.PRINT_NAME, A.CITY, A.COUNTRY,
	...
	I.CREATEDBY, I.PAID, I.MESSAGEID, I.METHODOFPAYID, I.METHOD_OF_PAY, DATE_FORMAT(I.ACHIEVED_DATE,'%d.%m.%Y') AS ACHIEVED_DATE, DATE_FORMAT(I.METHOD_OF_PAY_DATE,'%d.%m.%Y') AS METHOD_OF_PAY_DATE,
	...
	FROM addressbook AS A, invoice AS I WHERE A.MYID=$myID AND I.INVOICEID=$invoiceID");
...
if (!$query2)
	...
	foreach($query2 as $f)
	{
		...
		if(empty($AchievedDate))
		{
			$AchievedDate = $f['ACHIEVED_DATE'];
			$smarty->assign("ACHIEVED_DATE",$f['ACHIEVED_DATE']);
			$smarty->assign("AchievedDate",$f['ACHIEVED_DATE']);
		}
		else
		{
			$AchievedDate = $AchievedDate;
			$smarty->assign("ACHIEVED_DATE",$AchievedDate);
			$smarty->assign("AchievedDate",$f['ACHIEVED_DATE']);
		}
...
$PrintD = date("Y", strtotime($InvoiceDate));
$smarty->assign("PrintDate",$PrintD.'-'.$invoiceID);
$smarty->assign("CurrentInvoiceID","$invoiceID");
$smarty->assign("CreatedBy","$CreatedBy");

$posquery = $db->Execute("SELECT P.POSITIONID, P.POS_NAME, T.USERNAME, T.POSITIONID, T.POS_DESC, T.POS_QUANTITY, T.POS_PRICE, T.INVOICEID, T.TMP_INVOICEID, T.TAX, T.TAX_DIVIDE, T.TAX_MULTI, T.TAX_DESC, T.POS_GROUP FROM article AS P, tmp_invoice AS T WHERE P.POSITIONID=T.POSITIONID AND T.INVOICEID=$invoiceID ORDER BY T.POS_GROUP ASC, T.TMP_INVOICEID ASC");
...
$query4 = $db->GetRow("SELECT MIN(INVOICEID) AS MIN_INVOICEID FROM invoice");
if (!$query4)
	print($db->ErrorMsg());
else
...
//
$query5 = $db->GetRow("SELECT MAX(INVOICEID) AS MAX_INVOICEID FROM invoice");
if (!$query5)
	print($db->ErrorMsg());
else
...
//
$query6 = $db->Execute("SELECT METHODOFPAYID, DESCRIPTION FROM methodofpay ORDER BY DESCRIPTION ASC");

// Get the message from database
//
$query7 = $db->Execute("SELECT MESSAGEID, DESCRIPTION FROM message ORDER BY DESCRIPTION ASC");
