...
else
{
	...
	$query = $db->Execute("SELECT A.PREFIX, A.TITLE, A.FIRSTNAME, A.LASTNAME, A.ADDRESS, A.COMPANY, A.POSTALCODE, A.PRINT_NAME,
		A.CITY, A.COUNTRY, A.METHODOFPAY, A.MYID, DATE_FORMAT(I.ACHIEVED_DATE,'%d.%m.%Y') AS ACHIEVED_DATE, DATE_FORMAT(I.INVOICE_DATE,'%d.%m.%Y') AS INVOICE_DATE, I.INVOICEID, I.PAID, I.SUM_PAID, I.TOTAL_AMOUNT, I.MYID,
...
if(isset($tmpPos) && $tmpPos == '1')
{
	$posquery = $db->Execute("SELECT P.POSITIONID, P.POS_NAME, T.USERNAME, T.POSITIONID, T.POS_DESC, T.POS_QUANTITY, T.POS_PRICE, T.TAX, T.TAX_DIVIDE, T.TAX_MULTI, T.TAX_DESC, T.POS_GROUP, T.TMP_INVOICEID FROM article AS P, tmp_invoice AS T WHERE P.POSITIONID=T.POSITIONID AND T.USERNAME='$_SESSION[Username]' ORDER BY T.POS_GROUP ASC, T.TMP_INVOICEID ASC");
}
else
{
	$posquery = $db->Execute("SELECT P.POSITIONID, P.POS_NAME, V.POSITIONID, V.POS_DESC, V.INVOICEPOSID,
	V.POS_QUANTITY, V.POS_PRICE, V.POS_GROUP, V.INVOICEID, V.TAX, V.TAX_DIVIDE, V.TAX_MULTI, V.TAX_DESC FROM article AS P, invoicepos AS V WHERE P.POSITIONID=V.POSITIONID AND V.INVOICEID=$invoiceID ORDER BY V.POS_GROUP ASC, V.INVOICEPOSID ASC");
}
