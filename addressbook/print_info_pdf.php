...
$query = $db->Execute("SELECT A.PREFIX, A.FIRSTNAME, A.LASTNAME, A.TITLE, A.COMPANY, A.DEPARTMENT, A.ADDRESS,
	...
	BANKNAME, BANKACCOUNT, BANKNUMBER, BANKIBAN, BANKBIC, AES_DECRYPT(USERNAME,'$pkey') AS USERNAME, USERLANGUAGE, USER_ACTIVE
	FROM addressbook AS A, category AS C, methodofpay AS M WHERE A.CATEGORY=C.CATEGORYID AND A.METHODOFPAY=M.METHODOFPAYID AND A.MYID=$myID");
$row = $query->GetRows();
