...
$pdf->Line(20, $pdf->GetY(), 194, $pdf->GetY());
...
if (isset($Type) && $Type != 'DeliveryNote')
{
	...
	else
	{
		...
		if(!empty($Subtotal1))
		{
			$pdf->Row(array($a['invoice_subtotal'].':',Format_Number($Subtotal1)));
		}
	...
	if (isset($SUM_PAID) && $SUM_PAID > 0)
	{
		...
		{
			$pdf->Row(array($paidresult['PAYMENT_DATE'].' [ '. utf8_decode($paidresult['METHOD_OF_PAY']) .' ]'.' '.Format_Number($paidresult['SUM_PAID']).' '.$Currency));
		}
	}
}
...
if(isset($MESSAGEID))
	...
	$pdf->Row(array(utf8_decode($MESSAGEID)));
