			...
			<tr>
				<td class="phprechnung_tabelle">
					<table width="100%" border="0" cellspacing="3" cellpadding="3" summary="Tabelle 2">
						<tr>
						{if $TaxFree != 1}
							<td align="center" colspan="9"><h2>{$Offer} - {$New}</h2>
							</td>
						{else}
						...
						{if $smarty.session.NewID and ( $smarty.session.NewID eq 1 )}
							{if $TaxFree != 1}
								<tr>
									<td align="center" colspan="9" class="greentxt">{$NewEntry} {$Offer_No} {$offerID}
									</td>
								</tr>
							{else}
								<tr>
					...
					<table width="100%" border="0" cellspacing="1" cellpadding="3" summary="Tabelle 3">
						<tr class="mblueTD">
							<td nowrap="nowrap" align="left" valign="middle">{$PositionName}&nbsp;-&nbsp;<a title="{$PositionNew}" class="ninfolink" href="posnew.php?myID={$myID}&amp;messageID={$NR_MESSAGE}&amp;OfferDate={$OFFER_DATE}&amp;MethodOfPayment={$NR_METHOD_OF_PAYMENT}&amp;MethodOfPaymentDate={$MethodOfPaymentDate}&amp;Note={$Note}&amp;Order={$Order}&amp;Sort={$Sort}&amp;{$Session}">{$New}</a>
							</td>
