...
{else}
	<body onload="document.Edit.Pos_Name.focus();">
{/if}
...
<tr>
	...
	[&nbsp;<a title="{$BackMsg} - {$Position} - {$Searchresult}" class="ninfolink" href="searchlist.php?page={$page}{$Searchstring}&amp;Order={$Order}&amp;Sort={$Sort}&amp;{$Session}#{$posID}">{$BackMsg}</a>&nbsp;]
	</td>
</tr>
{else}
<tr>
	<td valign="middle" align="left" colspan="2">
	[&nbsp;<a title="{$BackMsg} - {$Position} - {$List}" class="ninfolink" href="list.php?page={$page}&amp;Order={$Order}&amp;Sort={$Sort}&amp;{$Session}#{$posID}">{$BackMsg}</a>&nbsp;]
	</td>
</tr>
{/if}
<tr><td align="center" colspan="2"><h2>{$Position} - {$Edit}</h2></td></tr>
<tr><td></td></tr>
<tr><td align="center" colspan="2" class="dbTxt">[ {$EntryNo} {$posID} ]</td></tr>
<tr><td></td></tr>
...
{foreach from=$PosGroupArray item=posgroup}
	{if $PosGroupID and ( $PosGroupID == $posgroup.POSGROUPID)}
...
<tr><td valign="top" align="right" width="40%">{$NoteMsg}:</td><td valign="top" align="left" width="60%"><textarea title="{$NoteMsg}" class="form_input" name="Note" rows="5" cols="37">{$Note}</textarea></td></tr>
<tr><td></td></tr>
