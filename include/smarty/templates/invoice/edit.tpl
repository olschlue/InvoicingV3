			...
			<tr>
				<td class="phprechnung_tabelle">
					<table width="100%" border="0" cellspacing="3" cellpadding="3" summary="Tabelle 2">
						{if $TaxFree neq 1}
							<tr>
								<td align="center" colspan="9"><h2>{$Invoice} - {$Edit}</h2>
							</td>
							</tr>
						{else}
							<tr>
						...
						<tr>
							...
							<form id="Edit" name="Edit" action="customerlist.php?{$Session}" method="post">
								...
								<input type="hidden" name="AchievedDate" value="{$ACHIEVED_DATE}" />
						...
						{if $MYID}
							...
							<tr>
								...
								<td align="right" valign="top" colspan="2">Leistungsdatum:
								</td>
								<td align="left" valign="middle" colspan="2">
								<form id="InvoiceA" name="InvoiceD" action="{$smarty.server.PHP_SELF}?{$Session}" method="post">
									<input type="hidden" name="myID" value="{$MYID}" />
									<input type="hidden" name="invoiceID" value="{$invoiceID}" />
									<input type="hidden" name="infoID" value="{$infoID}" />
									<input type="hidden" name="tmpID" value="{$invoiceID}" />
									...
									{/if}
									<input type="hidden" name="Note" value="{$NOTE}" />
									<input type="hidden" name="mark" value="InvoiceA.InvoiceAchieved" />
									<input type="hidden" name="page" value="{$page}" />
									<input type="hidden" name="Order" value="{$Order}" />
									<input type="hidden" name="Sort" value="{$Sort}" />
									<input type="hidden" name="Canceled" value="{$Canceled}" />
									<input type="text" name="AchievedDate" size="20" title="" value="{$ACHIEVED_DATE}" onchange="this.form.submit();" />
								</form>
								</td>
							</tr>
