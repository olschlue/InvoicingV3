	...
	{if $invoice.CANCELED neq 1}
		...
		{if $smarty.session.Username and ( $smarty.session.Username eq $Root or $smarty.session.Username eq $invoice.CREATEDBY)}
			{if $invoice.PAID eq '2'}
				...
				<td valign="top" align="center"><a href="copy_invoice.php?myID={$invoice.MYID}&amp;invoiceID={$invoice.INVOICEID}&amp;Type=Invoice&amp;{$Session}" target="_blank"><img border="0" src="../images/print1.png" title="{$Copy_Invoice}Kopiere - {$Invoice_No} {$invoice.INVOICEID}" alt="{$Copy_Invoice}Kopiere - {$Invoice_No} {$invoice.INVOICEID}" /></a></td>
