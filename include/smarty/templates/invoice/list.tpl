...
<tr><td align="center" colspan="9"><h2>{$Invoice} - {$List}</h2></td></tr>
...
{if $smarty.session.NewID and ( $smarty.session.NewID eq 1 )}
	<tr><td align="center" colspan="9" class="greentxt">{$NewEntry} {$Invoice_No} {$invoiceID}</td></tr>
{/if}
{if $smarty.session.EditID and ( $smarty.session.EditID eq 1 )}
	<tr><td align="center" colspan="9" class="greentxt">{$EntryChanged} {$Invoice_No} {$invoiceID}, {$Customer_No} {$myID}</td></tr>
{/if}
{if $smarty.session.CancelID and ( $smarty.session.CancelID eq 1 )}
	<tr><td align="center" colspan="9" class="greentxt">{$EntryCanceled} {$Invoice_No} {$invoiceID}, {$Customer_No} {$myID}</td></tr>
{/if}
{if $smarty.session.emailID and ( $smarty.session.emailID eq "1" )}
	<tr><td align="center" colspan="9" class="greentxt">{$Email_OK} {$FIRSTNAME} {$LASTNAME} {$COMPANY}, {$smarty.session.Type} {$invoiceID}, {$Customer_No} {$MYID}</td></tr>
{/if}
...
<td nowrap="nowrap" align="center" colspan="4">{$Entrys}:&nbsp;{$MaxRows}
{*
...
<a href="{$smarty.server.PHP_SELF}?Order={$Order}&amp;Sort={$Sort}&amp;Canceled=1&amp;{$Session}"><img border="0" src="../images/down.png" title="{$SortMsg} {$CanceledEntries}" alt="{$CanceledEntries}" /></a>
...
{foreach from=$InvoiceData item=invoice}
	...
	{if $invoice.CANCELED neq 1}
		...
		{if $smarty.session.Username and ( $smarty.session.Username eq $Root or $smarty.session.Username eq $invoice.CREATEDBY)}
			{if $invoice.PAID eq '2'}
				<td valign="top" align="center" width="50"><a href="edit.php?invoiceID={$invoice.INVOICEID}&amp;{$AddCurrentPage}Order={$Order}&amp;Sort={$Sort}&amp;Canceled={$Canceled}&amp;{$Session}"><img border="0" src="../images/edit.png" title="{$Editentry}" alt="{$Editentry}" /></a></td>
				<td valign="top" align="center" width="50"><a href="copy_invoice.php?myID={$invoice.MYID}&amp;invoiceID={$invoice.INVOICEID}&amp;Type=Invoice&amp;{$Session}" target="_blank"><img border="0" src="../images/print1.png" title="{$Copy_Invoice}Kopiere - {$Invoice_No} {$invoice.INVOICEID}" alt="{$Copy_Invoice}Kopiere - {$Invoice_No} {$invoice.INVOICEID}" /></a></td>
				<td valign="top" align="center" width="50"><a href="print_pdf.php?myID={$invoice.MYID}&amp;invoiceID={$invoice.INVOICEID}&amp;Type=Invoice&amp;{$Session}" target="_blank"><img border="0" src="../images/print.png" title="{$Print_Invoice} - {$Invoice_No} {$invoice.INVOICEID}" alt="{$Print_Invoice} - {$Invoice_No} {$invoice.INVOICEID}" /></a></td>
				<td valign="top" align="center" width="50"><a href="../payment/new.php?myID={$invoice.MYID}&amp;invoiceID={$invoice.INVOICEID}&amp;{$Session}"><img border="0" src="../images/bill.png" title="{$Invoice_Transaction} {$invoice.INVOICEID}{if $invoice.FIRSTNAME} {$invoice.FIRSTNAME}{/if}{if $invoice.LASTNAME} {$invoice.LASTNAME},{/if}{if $invoice.COMPANY} {$invoice.COMPANY}{/if}" alt="{$Invoice_Transaction} {$invoice.INVOICEID}{if $invoice.FIRSTNAME} {$invoice.FIRSTNAME}{/if}{if $invoice.LASTNAME} {$invoice.LASTNAME},{/if}{if $invoice.COMPANY} {$invoice.COMPANY}{/if}" /></a></td>
			{else}
