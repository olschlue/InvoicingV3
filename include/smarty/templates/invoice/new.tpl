...
<tr>
	<td id="td1_20" width="200px" valign="top">
	{* Menubar start *}
		<table border="0" width="80%" cellspacing="0" cellpadding="0" summary="Tabelle 4">
			...
			<tr>
				<td align="left" class="phprechnung_menu"><a accesskey="I" title="{$Invoice} - {$List}"
				href="list.php?{$Session}">{$Invoice}</a>
				</td>
			</tr>
			<tr>
				<td align="left" class="phprechnung_menu_sel"><a accesskey="1" title="{$Invoice} - {$New}"
	...
	{* Menubar end *}
	<td id="td1_2" width="20px">
	</td>
	<td style="width: auto;" valign="top" align="center">
				...
				<td class="phprechnung_tabelle">
					<table width="100%" border="0" cellspacing="3" cellpadding="3" summary="Tabelle 2">
						...
						{if $smarty.session.NewID and ( $smarty.session.NewID eq 1 )}
							...
							{else}
								<tr>
									<td align="center" colspan="7" class="greentxt">{$NewEntry} {$Invoice_No} {$invoiceID}
									</td>
								</tr>
							{/if}
						{/if}
						...
						<tr>
							...
							<form id="New" name="New" action="customerlist.php?{$Session}" method="post">
								...
								<input type="hidden" name="InvoiceDate" value="{$INVOICE_DATE}" /> <input type="hidden" name="AchievedDate" value="{$ACHIEVED_DATE}" />
					...
					<table width="100%" border="0" cellspacing="1" cellpadding="3" summary="Tabelle 3">
						<tr class="mblueTD">
							<td nowrap="nowrap" align="left" valign="middle">{$PositionName}&nbsp;-&nbsp;<a title="{$PositionNew}" class="ninfolink" href="posnew.php?myID={$myID}&amp;messageID={$NR_MESSAGE}&amp;InvoiceDate={$INVOICE_DATE}&amp;MethodOfPayment={$NR_METHOD_OF_PAYMENT}&amp;MethodOfPaymentDate={$MethodOfPaymentDate}&amp;Note={$Note}&amp;newofferID={$newofferID}&amp;Order={$Order}&amp;Sort={$Sort}&amp;{$Session}">{$New}</a>
							</td>
						...
						{foreach from=$PosData item=invoice}
							{if $smarty.session.Username and ( $smarty.session.Username eq $invoice.USERNAME )}
								<tr class="{cycle values="grayTD,wTD"}">
									...
									<td valign="top" align="center"><a href="posedit.php?tmpPosID={$invoice.TMP_INVOICEID}&amp;myID={$MYID}&amp;posID={$invoice.POSITIONID}&amp;messageID={$NR_MESSAGE}&amp;InvoiceDate={$INVOICE_DATE}&amp;MethodOfPayment={$NR_METHOD_OF_PAYMENT}&amp;MethodOfPaymentDate={$MethodOfPaymentDate}&amp;Note={$Note}&amp;newofferID={$newofferID}&amp;Order={$Order}&amp;Sort={$Sort}&amp;{$Session}"><img border="0" src="../images/edit.png" title="{$Position} - {$Edit}" alt="{$Position} - {$Edit}" /></a>
									</td>
									<td valign="top" align="center"><a href="posdelete.php?tmpPosID={$invoice.TMP_INVOICEID}&amp;myID={$MYID}&amp;messageID={$NR_MESSAGE}&amp;InvoiceDate={$INVOICE_DATE}&amp;MethodOfPayment={$NR_METHOD_OF_PAYMENT}&amp;MethodOfPaymentDate={$MethodOfPaymentDate}&amp;Note={$Note}&amp;newofferID={$newofferID}&amp;Order={$Order}&amp;Sort={$Sort}&amp;{$Session}"><img border="0" src="../images/delete.png" title="{$Position} - {$Delete}" alt="{$Position} - {$Delete}" /></a>
									</td>
							</tr>
							{/if}
						{/foreach}
						...
						<tr>
							...
							<input type="hidden" name="MethodOfPaymentDate" value="{$MethodOfPaymentDate}" />
							<input type="hidden" name="Note" value="{$Note}" />
							<input type="hidden" name="Mark" value="New1.SaveInvoice" />
							<input type="hidden" name="page" value="{$page}" />
							<input type="hidden" name="Order" value="{$Order}" />
							<input type="hidden" name="Sort" value="{$Sort}" />
							...
							<form id="Print" name="Print" action="print_pdf.php?{$Session}" target="_blank" method="post">
								...
								<input type="hidden" name="invoiceID" value="{$maxInvoiceID}" />
								<input type="hidden" name="myID" value="{$myID}" />
								<input type="hidden" name="InvoiceDate" value="{$INVOICE_DATE}" /> <input type="hidden" name="AchievedDate" value="{$ACHIEVED_DATE}" />
								<input type="hidden" name="MethodOfPayment" value="{$NR_METHOD_OF_PAYMENT}" />
								<input type="hidden" name="MethodOfPaymentDate" value="{$MethodOfPaymentDate}" />
								...
								{else}
									<input class="button" type="submit" title="{$Print_Invoice}" value="{$Print}" disabled="disabled" />
								{/if}
							</form>
							</td>
							{if $TaxFree neq 1}
							...
							{if $infoID eq 9}
								<form id="Searchlist" name="Searchlist" action="searchlist.php?{$Session}#{$invoiceID}" method="post">
									...
									<input type="hidden" name="DateFrom1" value="{$DateFrom1}" /> <input type="hidden" name="DateAchieved1" value="{$DateAchieved1}" />
							...
							<form id="New1" name="New1" action="newf.php?{$Session}" method="post">
								...
								<input class="button" name="SaveInvoice" type="submit" title="{$Save_Invoice}" value="{$Save}" />
							</form>
							</td>
						</tr>
					</table>
...
<tr>
	<td id="td2_20" width="200px"><br />
	</td>
	<td id="td2_2" width="20px">
	</td>
	<td style="width: auto;" valign="top"><br />
	</td>
</tr>
</table>
